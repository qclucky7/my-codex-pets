Create one horizontal animation strip for Codex pet `meining`, state `jumping`.
...
Animation continuity: keep apparent pet scale, head size, body height, and foot baseline identical to the approved `idle` baseline in every frame. This custom `jumping` row has no vertical jump and no scale change.

State action: Gentle cheek-resting loop while standing still; this custom row intentionally replaces jumping.
...
- Animate a gentle cheek-resting gesture: raise one hand toward the cheek, softly rest the cheek in the palm, hold with a warm thoughtful expression, then return toward the composed pose.
- The supporting hand must remain anatomically connected to the arm; the other hand stays relaxed near the waist or sleeve.
- Preserve the face shape while the cheek rests in the palm; do not squash, stretch, or enlarge the head.
