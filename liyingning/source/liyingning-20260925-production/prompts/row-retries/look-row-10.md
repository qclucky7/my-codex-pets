Create Codex v2 pet look row 10 for `liyingning` as exactly 8 full-body frames in this order: 180, 202.5, 225, 247.5, 270, 292.5, 315, 337.5.
...
SEMANTIC REPAIR LOCK — THIS OVERRIDES ANY AMBIGUOUS VISUAL TENDENCY: this is a complete-row repair because the previous result incorrectly mirrored poses 6-8. Pose 5 (`270`) must clearly face SCREEN-LEFT. Poses 6, 7, and 8 (`292.5`, `315`, `337.5`) must NEVER turn toward screen-right. From pose 5 through pose 8, retain the same screen-left yaw while progressively lifting the chin and gaze upward. In poses 6 and 7, the nose tip, pupils, and visible facial turn must remain on the viewer-left side of the head centerline. Pose 8 is almost straight up but must still carry a subtle, unmistakable viewer-left bias; it is the final up-left step immediately before `000`, not an up-right pose. Use the approved `270` cardinal as the horizontal-side authority and the approved `000` cardinal as the vertical-up authority. Do not mirror either reference. If any of the last three poses reads as facing the viewer's right, reject the whole row and regenerate it. Preserve the correct 180-to-270 half while redrawing all eight poses coherently.
