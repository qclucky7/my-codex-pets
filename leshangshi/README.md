...
```powershell
$target = Join-Path $HOME ".codex\pets\leshangshi"
New-Item -ItemType Directory -Path $target -Force | Out-Null
Copy-Item .\leshangshi\pet.json, .\leshangshi\spritesheet.webp -Destination $target -Force
```
...
|   `-- waving.gif
`-- source/
    |-- leshangshi-20260810-base-selection/
    `-- leshangshi-20260810-production/
```
...
`pet.json` 和 `spritesheet.webp` 是安装所需的发布文件；`source/leshangshi-20260810-production` 保存可追溯的生成输入、中间产物与 QA 证据，不参与日常安装。
