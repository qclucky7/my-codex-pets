Create Codex v2 pet look row 10 for `ziling` as exactly 8 full-body frames in this order: 180, 202.5, 225, 247.5, 270, 292.5, 315, 337.5.
...
CRITICAL LOOP-BOUNDARY REPAIR: after `270` screen-left, make `292.5`, `315`, and `337.5` visibly and evenly return from left profile toward the approved front/up family. Do not repeat three side profiles. `337.5` must be nearly front-facing with eyes up-left and only one small step from `000 up`; match the `000` body area, scale, baseline, horizontal center, and head angle closely enough to loop without a visible snap.
