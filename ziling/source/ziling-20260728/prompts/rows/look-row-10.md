Create one horizontal look-direction strip for Codex pet `ziling`, atlas row 10.
...
CRITICAL LOOP-BOUNDARY REPAIR: after the `270` screen-left cardinal, frames `292.5`, `315`, and `337.5` must progressively rotate the face and head back toward the front/up pose by equal visible steps. They must not remain three near-identical left-facing profiles. `337.5` must be a clear near-up, slight screen-left three-quarter pose—only one step from the approved `000 up` anchor—with nearly front-facing shoulders, body area and center close to `000`, and eyes distinctly up-left. The `337.5 -> 000` transition must not produce an abrupt size, center, silhouette, or head-angle jump.
