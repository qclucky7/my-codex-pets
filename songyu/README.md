...
造型采用适合桌面小尺寸显示的 Q 版中国 3D 动漫风格，保留宋玉的青白渐变仙裙、白玉发冠、侧边玉饰、深色长发与温柔眉眼。鼠标悬停触发她的标志性吹茶动作；左右拖动时，她会单手掐剑诀御空飞行，另一只手独立后展，发丝、衣袖和裙摆随飞行方向自然后扬，不使用遁光球。
...
| --- | --- |
| `idle` | 呼吸、眨眼与发丝微动 |
| `running-right` | 单手掐诀向右御空飞行，另一只手与衣袂向左后扬 |
| `running-left` | 单手掐诀向左御空飞行，另一只手与衣袂向右后扬 |
| `waving` | 温柔挥手问候 |
