Create Codex v2 pet look row 10 for `songyu` as exactly 8 full-body frames in this order: 180, 202.5, 225, 247.5, 270, 292.5, 315, 337.5.
...
CRITICAL REPAIR: after the clear `270 screen-left` pose, make `292.5` left-and-starting-up, `315` balanced up-left, and `337.5` strongly up-left exactly one step before approved `000 up`. In `337.5`, eyes and chin are clearly raised while nose tip and pupils remain slightly left of head center. Do not turn neutral/front, flatten the last three poses, snap, or reverse.

CRITICAL SCALE REPAIR: the rejected row was 25–40% smaller in visible area than row 9. Match the attached completed row 9 at source scale: identical full-body height, head diameter, shoulder/torso/skirt width, hair volume, facial feature size, and planted lower-body position. Make `180` nearly equal in silhouette area and center to `157.5`, and `337.5` nearly equal to `000`. Never draw a smaller, narrower, or more distant model.
