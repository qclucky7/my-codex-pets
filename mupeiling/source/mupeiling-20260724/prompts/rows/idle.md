Create one horizontal animation strip for Codex pet `mupeiling`, state `idle`.
...
Cross-row scale lock: this idle row defines the canonical full-body height, head size, shoulder width, foot baseline, and three-head-tall proportions for the entire pet. Keep generous but compact padding so directional movement rows can use exactly this same character size.
