Create one horizontal look-direction strip for Codex pet `yuanyao`, atlas row 9.
...
`000` UP IS A HARD GATE: copy the direction meaning and pose family of the approved first cardinal, not a neutral/front expression. In the first frame Yuan Yao's blue-violet pupils must sit visibly high within both eye apertures, the lower eyelids and lower eye whites must support the upward gaze, her chin must lift, and her face must pitch upward enough to read as looking above the top edge at final pet size. Frame 1 must be visibly different from idle/front. Frames 2-4 then reduce that vertical-up cue evenly while adding screen-right turn, reaching the unmistakable right cardinal at frame 5. Reject any row whose first frame is frontal, level-eyed, or downward-looking.
